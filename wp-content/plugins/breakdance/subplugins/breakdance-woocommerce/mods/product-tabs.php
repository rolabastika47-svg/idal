...
function tabs($propertiesData = [])
{
    ...
    foreach ($product_tabs as $key => $product_tab) {
        ...
        $tabs[] = getTabData($custom_tab, $product_tab, $key);
    }
    ...
    foreach ($product_tabs as $key => $product_tab) {
        echo '<div tabindex="0" role="tabpanel" class="bde-tabs__panel js-panel" id="tab-panel-' . $tabs_id . '-' . $i . '" aria-labelledby="tab-' . $tabs_id . '-' . $i . '">';
 ...
 * @return array
 */
function getTabData($data, $productTab, $id)
{
