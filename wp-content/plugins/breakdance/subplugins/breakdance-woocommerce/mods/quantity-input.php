...
{
    wp_enqueue_script('breakdance-woo-qty', BREAKDANCE_WOO_URL . 'js/quantity.js', [], __BREAKDANCE_VERSION, true);
}
...
function addQuantityInputToMiniCart($html, $cart_item, $cart_item_key)
{
    ...
    if ($_product->is_sold_individually()) {
        $input = sprintf('<span></span><input type="hidden" name="cart[%s][qty]" value="1" />', $cart_item_key);
    } else {
...
add_action('woocommerce_after_shop_loop_item', function () {
    ...
    bdox_run_action('breakdance_shop_loop_footer');
    echo '</div>';
});
