...
function register_handler($route, callable $callback, $minimum_permissions = 'never', $any_url = false, $options = [], $skip_nonce_check = false)
{
    ...
    if ($minimum_permissions === 'never') {
        // bad request
        throw new Exception(esc_html__('You must specify the minimum permissions to access your AJAX action.', 'breakdance'), 500);
    }
    ...
    $handler = function () use ($callback, $any_url, $minimum_permissions, $route, $options, $skip_nonce_check) {
        try {
            ...
            if (!$allowed) {
                if (!is_user_logged_in()) {
                    throw new BreakdanceAjaxHandlerException(
                        sprintf(
                            ...
                            esc_html__('You need to be logged in and have minimum permission level of "%s"', 'breakdance'),
                            $minimum_permissions
                        ),
                        401
                    );
                ...
                // forbidden request
                $error = sprintf(
                    /* translators: 1: Action/route name, 2: Required permission level */
                    esc_html__('Not enough permissions to perform the action "%1$s", minimum permission level is "%2$s".', 'breakdance'),
                    $route,
                    $minimum_permissions
                );
            ...
            if (!$isHandlerForRemoteRequest && !$skip_nonce_check) {
                ...
                if (!$nonceTick) {
                    throw new BreakdanceAjaxHandlerException(
                        esc_html__('Failed security check', 'breakdance'),
                        419,
                        null,
                        esc_html__('Expired', 'breakdance')
                    );
                }
            ...
            if (!empty($options['args'])) {
                ...
                if ($callback_args_filtered === false || $callback_args_filtered === null) {
                    $error_message = !is_breakdance_development_environment()
                        ...
                        : esc_html__('All of the required POST parameters are missing', 'breakdance');
            ...
            if (is_array($return_value) && array_key_exists('error', $return_value)) {
                throw new BreakdanceAjaxHandlerCustomException((string) $return_value['error'], 500);
            } elseif (!is_array($return_value) && !$return_value) {
                $return_value = (object) [];
            } elseif (!is_array($return_value)) {
                $error = sprintf(
                    /* translators: 1: Return value type, 2: Route name */
                    esc_html__('AJAX callbacks should return an associative array (["key" => "value") or a blank array ([]). Got %1$s for route "%2$s"', 'breakdance'),
            ...
            if ($unexpected_output) {
                $error = esc_html__('Unexpected output during AJAX request. AJAX Callbacks should return an associative array.', 'breakdance') . ' --------- ' . wp_strip_all_tags(
                    $unexpected_output
                );
                // bad request
                throw new BreakdanceAjaxHandlerException($error, 500);
...
add_action(
    ...
    static function ($userLogin) {
        // In previous versions, we accidentally set a transient for non-full users.
...
function broadcastNonce()
{
    ...
    $hasRefreshNonceQueryParam = (bool) filter_input(INPUT_GET, 'breakdanceRefreshSession', FILTER_VALIDATE_BOOLEAN);
...
JS;
        ...
        if ($hasRefreshNonceQueryParam) {
            add_action('admin_notices', static function () {
                ...
                echo '<div class="updated"><p>' . sprintf(esc_html__('Your %s session token has been refreshed', 'breakdance'), __bdox('plugin_name')) . '</p></div>';
            });
        }
    }
}
...
/**
 * Delete all transients that start with breakdance_nonce_updated_
 */
function delete_all_nonce_updated_transients()
{
    global $wpdb;

    $option_name = '_transient_breakdance_nonce_updated_%';
