 ...
 * @return bool
 */
function is_current_url($menu_item_url)
{
    if (!$menu_item_url) return false;
    ...
    // *Breakdance*
    // We purposely don't support anchor links. This is necessary in order to avoid breaking backwards compatibility with 2.4 and older.
    if (strpos($menu_item_url, '#')) {
        return false;
    }
...
\Breakdance\PluginsAPI\PluginsController::getInstance()->registerTwigFunction(
    'is_current_url',
    'Breakdance\PluginsAPI\is_current_url',
    '(url) => {
        return false;
    }',
    false
 ...
 * @return mixed
 */
function getAttachmentAlt($id)
{
    if ($id) {
        return get_post_meta((int) $id, '_wp_attachment_image_alt', true);
    }
...
\Breakdance\PluginsAPI\PluginsController::getInstance()->registerTwigFunction(
    'get_attachment_alt',
    '\Breakdance\PluginsAPI\getAttachmentAlt',
    '(id) => {
        // 1. Fetch image alt from the server
        // 2. Find the image element by alt using the attachment id
        // document.querySelector(`[alt="bde-image-${id}"]`).alt = "New Alt From Server";
        // 3. Store the alt value in a global variable.
        // 4. If alt is present in cache, return it instead of fetching from the server.
        // return `bde-image-${id}`;
        return `{placeholder}`;
    }',
    false
...
/**
 * Removes whitespace and line breaks from the beginning and end of a string
 *
 * @param string $string
 * @return string
 */
...
\Breakdance\PluginsAPI\PluginsController::getInstance()->registerTwigFunction(
    'trim_whitespace',
    'Breakdance\PluginsAPI\trimWhitespace',
