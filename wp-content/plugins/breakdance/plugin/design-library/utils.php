 ...
 * @return bool
 */
function doesDesignLibraryRelyOnDesignPresets()
{
    return \Breakdance\Data\get_global_option('design_library_relies_on_design_presets') == 'yes';
}

/**
 * @return bool
 */
function isDesignLibraryFullSiteImportEnabled()
{
    return \Breakdance\Data\get_global_option('design_library_full_site_import_enabled') == 'yes';
}

/**
 * @return string
 */
function getDesignLibraryFullSiteImportThumbnailUrl()
{
    return (string) \Breakdance\Data\get_global_option('design_library_full_site_import_thumbnail_url');
}
...
function getCopyableElements()
{
    ...
    /** @var string[] */
    return bdox_run_filters('breakdance_design_library_copyable_elements', $elements);
}
...
function fetchDesignProviders()
{
     ...
     * @var string|false
     */
    $design_providers_url = bdox_run_filters('breakdance_default_design_library_providers_json_url', 'https://breakdance.com/wp-content/uploads/breakdance/design_sets/design_library_providers.json');

    $response = null;

    if ($design_providers_url) {
        $response = wp_remote_request($design_providers_url);
    }
    ...
    if (count($designSets)) {
        $designSets = array_map(
            function ($url) {
                ...
                if ($set['fullSiteImportEnabled'] ?? false) {
                    $designSet['type'] = "complete_site";
                    $designSet['fullSiteImportEnabled'] = true;
                    $designSet['meta'] = [
                        'thumbnailUrl' => $set['fullSiteImportThumbnailUrl'] ?? ""
                    ];
                }
    ...
    $allProviders = array_merge($providers, $designSets ?: []);
...
function getInvalidDesignSets()
{
    ...
    return array_values(
        array_filter(
            $providers,
            fn($provider) => !isValidDesignSet($provider, true)
        )
    );
}
 ...
 */
function getDesignSetRemoteData($url, $skipCache = false)
{
    $cache_key = 'breakdance_design_set_' . $url;

    /**
     * @var DesignSetData|false
     */
    $cache = get_transient($cache_key);

    if ($cache !== false && !$skipCache) {
        /** @var DesignSetData */
        return $cache;
    }
    ...
    // Cache is set to expire after 25 hours to ensure a small overlap with the cron job,
    // which updates the data every 24 hours. This ensures the cache is always refreshed
    // before it expires, preventing the http requests from slowing down the builder.
    /**
     * @psalm-suppress MixedArgument
     * @psalm-suppress UndefinedConstant
     */
    ...
    return [
        ...
        'meta_query' => [
            ...
            [
                'relation' => 'OR',
                [
                    'key' => __bdox('_meta_prefix') . 'hide_in_design_set',
                    'compare' => 'NOT EXISTS',
                ],
                [
                    'key' => __bdox('_meta_prefix') . 'hide_in_design_set',
 ...
 */
function getPostsForDesignSet($includeDraft = true)
{
    /** @var \WP_Post[] */
    $posts = get_posts(getArgumentsForDesignSetPostsQuery($includeDraft));

    return array_map(function ($post) {
        /** @var string */
        $rawTags = get_post_meta($post->ID, __bdox('_meta_prefix') . 'tags', true);
        /** @var string[] */
 ...
 * @return bool
 */
function isValidDesignSet($url, $skipCache = false)
{
    if ($url === '') {
        return false;
    }
    ...
    $response = getDesignSetRemoteData($url, $skipCache);
 ...
 * @return bool
 */
function hideDesignLibrary()
{
    return \Breakdance\Data\get_global_option('design_library_hide_on_this_website') == 'yes';
}

/**
 * @return bool
 */
function hideDesignLibraryInBuilder()
{
    return \Breakdance\Data\get_global_option('design_library_hide_in_builder') == 'yes';
}
