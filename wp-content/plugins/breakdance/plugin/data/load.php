...
function load_document()
{
    ...
    if (isset($_POST['id'])) {
        ...
        $lastPreviewedItem = get_meta($id, __bdox('_meta_prefix') . 'template_last_previewed_item') ?: false;
     ...
     * @psalm-suppress MixedAssignment
     */
    $aiSettings = function_exists('\Breakdance\AI\getSettings') ? \Breakdance\AI\getSettings()['data'] : false;

    /** @psalm-suppress MixedAssignment */
    $futureLayerSettings = function_exists('getFutureLayerSettings')
...
add_action('breakdance_loaded', function () {
    \Breakdance\AJAX\register_handler(
        'breakdance_load_presets',
        '\Breakdance\Data\load_presets',
        'edit'
    );
});
...
{
    return \Breakdance\DesignPresets\getPresetsDataForBuilder();
}
...
function get_needed_data()
{
    ...
    return [
        ...
        'breakdanceSettingsUrl' => get_admin_url() . 'admin.php?page=' . __bdox('admin_page_settings_slug'),
...
function get_needed_urls_for_post($post_id, $previewableItems, $lastPreviewedItem)
{
     ...
     */
    if (
        !in_array($postType, (array) BREAKDANCE_DYNAMIC_DATA_PREVIEW_POST_TYPES, true) &&
        !$lastPreviewedItem &&
        $previewableItems &&
    ...
    if ($postType === BREAKDANCE_PART_POST_TYPE) {
        $backToWordpressUrl = get_edit_post_link($post_id, '');
    }
