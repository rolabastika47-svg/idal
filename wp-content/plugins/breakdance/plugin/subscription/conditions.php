...
function getProOnlyTemplateConditionMessageIfConditionsContainProOnly($templateId, $label)
{
    ...
    if (is_array($templateSettings) && conditionsContainProOnly($templateSettings['ruleGroups'] ?? null)) {
        if ($label) {
            /* translators: %s is the template label/name */
            return getFreeModeErrorMessage(sprintf(__('A pro condition was used in this <b>%s</b>.', 'breakdance'), $label));
        } else {
            return getFreeModeErrorMessage(sprintf(__('A pro condition was used.', 'breakdance')));
        }
