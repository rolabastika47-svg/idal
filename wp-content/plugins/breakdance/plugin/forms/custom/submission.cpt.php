 ...
 * @return string
 */
function getFormSubmissionCapability()
{
    /** @var string $cap */
    $cap = bdox_run_filters('breakdance_form_submission_capability', 'manage_options');
    return $cap;
}
...
{
    $cap = getFormSubmissionCapability();
...
function restrictAccess()
{
    ...
    if ($pagenow == 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'breakdance_form_res') {
        if (!canViewSubmissions()) {
            wp_die(__('Sorry, you are not allowed to edit posts in this post type.'));
        }
    }

    // Check if the current user is on the post edit screen
    if ($pagenow == 'post.php' && isset($_GET['post'])) {
        $post_id = (int) $_GET['post'];
        /** @var \WP_Post|null $post */
        $post = get_post($post_id);

        if ($post && $post->post_type == 'breakdance_form_res') {
            if (!canViewSubmissions()) {
...
add_action('admin_init', '\Breakdance\Forms\Submission\restrictAccess');
...
function enqueueStyles($hook)
{
    ...
    if ($hook == 'post-new.php' || $hook == 'post.php') {
        if ('breakdance_form_res' === $post->post_type) {
            ...
            wp_enqueue_style('breakdance-submission', $url, [], $version);
        }
    }
}
 ...
 */
function getSubmissionSettings($id)
{
    /** @var FormSettings|string|null $settings */
    $settings = get_post_meta($id, __bdox('_meta_prefix') . 'form_settings', true);
...
function getMeta($id)
{
    $formId = (int) get_post_meta($id, __bdox('_meta_prefix') . 'form_id', true);
    $postId = (int) get_post_meta($id, __bdox('_meta_prefix') . 'post_id', true);
    /** @var FormUserSubmittedContents $fields */
    $fields = get_post_meta($id, __bdox('_meta_prefix') . 'fields', true) ?: [];
    /** @var FormSettings|null $settings */
    $settings = getSubmissionSettings($id);

    $ip = (string) get_post_meta($id, __bdox('_meta_prefix') . 'ip', true);
    $referer = (string) get_post_meta($id, __bdox('_meta_prefix') . 'referer', true);
    $userAgent = (string) get_post_meta($id, __bdox('_meta_prefix') . 'user_agent', true);
    $userId = (int) get_post_meta($id, __bdox('_meta_prefix') . 'user_id', true);
    ...
    $date = get_the_time('F j, Y \a\t h:i a', $id);
    $modified = get_the_modified_date('F j, Y \a\t h:i a', $id);
...
/**
 * @param array{type: string, message: string, executed_at?: string} $action
 * @return string
 */
 ...
 * @return string
 */
function getActionNoticeClass($action)
{
    if ($action['type'] !== 'success') {
...
?>
    <div class="breakdance-actions">
        ...
        ?>
                ...
                <div>
                    <?php foreach ($action['context'] as $context) {
                        ...
                        <dl class="breakdance-action-context-list">
                            ...
                            ?>
                                <dt class="breakdance-action-context-list__title"><?php echo esc_html((string) $contextKey); ?></dt>
                                <dd class="breakdance-action-context-list__details">
                                    <?php echo !is_string($contextValue) ? '<pre>' . esc_html(print_r($contextValue, true)) . '</pre>' : esc_html($contextValue); ?>
                                </dd>
                            <?php } ?>
                        </dl>
                    <?php } ?>
            ...
            <div class="breakdance-action-item breakdance-action-item--<?php echo getActionStatus($action); ?>">
                <header class="breakdance-action-item-header">
                    <h3 class="breakdance-action-item__name"><?php echo $name; ?></h3>
                    <span class="breakdance-action-item__icon dashicons <?php echo getActionIcon($action); ?>"></span>
                    ...
                    <?php if (!empty($action['context'])) { ?>
                        <span class="breakdance-action-item__view">
                            <a href="#TB_inline?&width=750&height=640&inlineId=inspect-action-<?php echo $slug; ?>" class="thickbox" name="<?php echo $name; ?>">Details</a>
                        </span>
                    <?php } ?>
                </header>

                <p class="breakdance-action-item__message notice <?php echo getActionNoticeClass($action); ?> ">
                    <?php echo $message; ?>
                </p>
            </div>
        <?php } ?>
...
function addMetaBoxes()
{
    ...
    if ($post !== null) {
        ...
        $uploads = get_post_meta($post->ID, __bdox('_meta_prefix') . 'uploads', true);
...
function onSubmissionSaved($postId, $post, $update)
{
    ...
    update_post_meta($postId, __bdox('_meta_prefix') . 'fields', $fields);
}
...
function parseFormFilter($query)
{
    ...
    $query->query_vars['meta_query'][] = [
        ['key' => __bdox('_meta_prefix') . 'post_id', 'value' => $formId['postId']],
        ['key' => __bdox('_meta_prefix') . 'form_id', 'value' => $formId['formId']],
    ];
}
...
function getFormNameFromLatestSubmissionSettings($postId, $formId)
{
    $latestSubmissions = wp_get_recent_posts([
            ...
            'posts_per_page' => 1,
            'meta_query' => [
                    ['key' => __bdox('_meta_prefix') . 'form_id', 'value' => $formId],
                    ['key' => __bdox('_meta_prefix') . 'post_id', 'value' => $postId],
            ]
    ], 'OBJECT');
    if ($latestSubmissions && isset($latestSubmissions[0])) {
        ...
        if ($latestSubmission->ID) {
            /** @var FormSettings|null $settings */
            $settings = getSubmissionSettings($latestSubmission->ID);
...
function getUniqueFormsFromSubmissions()
{
    ...
    $query = "
        ...
        JOIN $wpdb->postmeta p2
           ...
           AND p1.meta_key = '{$prefix}form_id'
           AND p2.meta_key = '{$prefix}post_id'";
...
{
    add_post_meta((int) $id, __bdox('_meta_prefix') . 'form_actions', $response, true);
}
...
{
    $formPostId = (int)get_post_meta($post->ID, __bdox('_meta_prefix') . 'post_id', true);
    $formId = (int)get_post_meta($post->ID, __bdox('_meta_prefix') . 'form_id', true);
    ...
    ?>
        <table class="form-table">
            ...
            foreach ($attachments as $attachment) {
                /** @var FormFile $file */
                $file = get_post_meta($attachment->ID, __bdox('_meta_prefix') . 'file', true);
                ?>
                    <tr>
                        <td>
...
function addCsvExportButton($where)
{
    ...
    if ($post_type_object->name === 'breakdance_form_res') {
        ...
        /** @var array $urlArgs */
        $urlArgs = bdox_run_filters('breakdance_add_csv_export_url_args', ['breakdance_action' => 'export_submissions_to_csv', '_wpnonce' => $wp_nonce]);
