...
abstract class ApiAction extends Action
{
    ...
    protected function handleResponse($response)
    {
        if ($response instanceof \WP_Error) {
            if ($response->get_error_code() === 'http_request_failed'){
                return ['error' => esc_html__('Error requesting data. The service may be down, your settings may be wrong, or there\'s a problem with your server', 'breakdance')];
            }
        ...
        if (wp_remote_retrieve_response_code($response) >= 300) {
            return ['error' => $this->handleErrorMessage($responseData)];
        }
    ...
    protected function handleErrorMessage($response)
    {
        ...
        return esc_html__('Error accessing resource', 'breakdance');
    }
    ...
    protected function getResponseBodyOrMessage($response)
    {
        ...
        try {
            ...
            return ['error' => esc_html__('Invalid response', 'breakdance')];
        }
    }
     ...
     * @param string|null $fallbackApiKeyName
     * @return string|null
     */
    public static function getApiKeyFromApiKeyInput($apiKeyInput, $apiKeyName, $fallbackApiKeyName = null)
    {
        if (!$apiKeyInput) {
        ...
        if ($apiKeyInput['type'] === 'default') {
            ...
            $apiKey = $apiKeyInput[$fallbackApiKeyName ?? 'apiKey'];
        }
