...
function maybe_override_the_theme_with_a_breakdance_template($file_to_include)
{
    ...
    if (function_exists('\Breakdance\MaintenanceMode\visitorShouldSeeAMaintenancePage') && \Breakdance\MaintenanceMode\visitorShouldSeeAMaintenancePage()) {
    ...
    /** @var boolean $shouldOverrideTemplate */
    $shouldOverrideTemplate = bdox_run_filters('breakdance_should_override_template', true, $file_to_include);
...
{
    return isRequestFromBuilderIframe() && shouldNotRenderHeaderOrFooterForCurrentPostType();
}
