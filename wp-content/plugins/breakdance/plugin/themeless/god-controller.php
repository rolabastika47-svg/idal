...
function missingTemplateError($originalTemplateHierarchyForRequest, $missingPostId, $missingPostIsTrashed) {
    ...
    echo '<h4>' . esc_html__('Template Hierarchy Error', 'breakdance') . '</h4>';

    /* translators: %d is the template ID */
    echo sprintf(esc_html__('There is no template with ID %d.', 'breakdance'), (int) $missingPostId) . '<br /><br />';
    ...
    for ($i = 0; $i < count($originalTemplateHierarchyForRequest); $i++) {
        ...
        if ($originalTemplateHierarchyForRequest[$i + 1] === $missingPostId) {
            ...
            $impossibleBecause = $missingPostIsTrashed
                ? __("This is impossible, since that template has been moved to trash", 'breakdance')
                : __("This is impossible, since that template doesn't exist", 'breakdance');

            echo wp_kses_post(sprintf(
                /* translators: 1: Template title, 2: Template ID, 3: Missing template ID */
                __('But template <em>%1$s (ID %2$d)</em> tries to inherit its design from template %3$d.', 'breakdance'),
                esc_html($postTitle),
                (int) $postId,
                (int) $missingPostId,
            ));

            echo esc_html($impossibleBecause) . '<br /><br />';

            echo wp_kses_post(
                sprintf(
                /* translators: 1: Template title, 2: Template ID */
                __('To fix, edit template <em>%1$s (ID %2$d)</em>, and clear the <i>Inherit Design From</i> field or choose a template that exists.', 'breakdance'),
                esc_html($postTitle),
                (int) $postId
            )) . '<br /><br />';

            echo '<a force-allow-clicks target="_blank" href="' . esc_url($manageTemplatesUrl) . '">' . esc_html__('Manage Templates', 'breakdance') . '</a>';
