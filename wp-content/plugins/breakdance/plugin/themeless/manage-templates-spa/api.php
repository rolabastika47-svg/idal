...
function getTemplatesByPostType($postType)
{
    ...
    if ($postType === BREAKDANCE_TEMPLATE_POST_TYPE) {
        ...
        return ['error' => __("Can't get data for an unsupported post type", 'breakdance')];
    }
...
function duplicateTemplate($originalId, $title, $settings, $postType)
{
    ...
    /** @var array $data */
    $data = get_meta($originalId, __bdox('_meta_prefix') . 'data');

    set_meta($response['data']['id'], __bdox('_meta_prefix') . 'data', $data);
...
function saveTemplate($title, $id, $settings, $postType)
{
    ...
    if (is_wp_error($result)) {
        ...
        return ['error' => sprintf(__("Couldn't update template with ID \"%s\"", 'breakdance'), $id)];
    }

    /**  @var int */
    $result = $result;

    \Breakdance\Data\set_meta($result, __bdox('_meta_prefix') . 'template_settings', $settings);

    $template = get_post($result);

    if (!$template) {
        return ['error' => sprintf(__("Couldn't get template with ID \"%s\"", 'breakdance'), $id)];
    }
...
function deleteTemplate($id, $trashOrDelete)
{
    ...
    if (!$result) {
        ...
        if ($trashOrDelete === 'delete') {
            /* translators: %s: template ID */
            return ['error' => sprintf(__("Couldn't delete template with ID \"%s\"", 'breakdance'), $id)];
        } else {
            /* translators: %s: template ID */
            return ['error' => sprintf(__("Couldn't trash template with ID \"%s\"", 'breakdance'), $id)];
        }
...
function restoreTemplate($id)
{
    ...
    if (!$result) {
        /* translators: %s: template ID */
        return ['error' => sprintf(__("Couldn't restore template with ID \"%s\"", 'breakdance'), $id)];
    }
...
function emptyTrashByTemplatePostType($postType)
{
    ...
    if ($failedToDeleteSomething) {
        return ['error' => __('Failed to empty trash', 'breakdance')];
    }
