...
function getTemplateForRequest($templates)
{
    ...
    if (\Breakdance\isRequestFromBuilderIframe() && isset($_GET['breakdance_open_document'])) {
        ...
        if (shouldNotRenderTemplateCurrentPostType($postType)) {
            return false;
        }
...
function _getAllTemplatesForRequest($templates, $useFallbackTemplates = false)
{
    ...
    $templatesThatApply = array_filter(
        $templates,
...
{
    if (
        ! isset($template['settings'], $template['settings']['type'])
        || ! $template['settings']
    ) {
        return false;
    }
    ...
    $evaluatedRuleGroups = array_map(
        '\Breakdance\Themeless\doesRuleGroupApply',
        $ruleGroups
    );
...
function doesRuleApply($rule)
{
     ...
     */
    return (bool)bdox_run_filters('breakdance_themeless_request_does_rule_apply', $doesConditionPass, $maybeCondition, $rule);
}
...
function findTemplateType($slug, $templateTypeCategories)
{
    ...
    $matchingTemplateTypes = array_filter(
        $templateTypes,
