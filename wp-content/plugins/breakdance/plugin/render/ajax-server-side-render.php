...
function serverSideRender($propertiesJsonString, $parentPropertiesJsonString, $elementSlug)
{
    ...
    // https://github.com/soflyy/breakdance/issues/6483

    /** @var string $nonce */
    $nonce = $_GET['_ajax_nonce'] ?? '';
    if (!wp_verify_nonce($nonce, 'breakdance_ajax')) {
        status_header(401);
        exit('AJAX nonce is missing');
    }
    ...
    if ($elementSlug && class_exists($elementSlug)) {
         ...
         */
        $ssrResult = bdox_run_filters("breakdance_builder_ssr_rendered_html", $ssrResult, $properties, $elementSlug, $parentProperties);
