...
use const Breakdance\Filesystem\OXYGEN_UPLOADS_SUBDIR_REL_PATH;
...
function get_uploads_dir_abs_path(): string
{
    ...
    $prefix = BREAKDANCE_MODE === 'oxygen' ? OXYGEN_UPLOADS_SUBDIR_REL_PATH : BREAKDANCE_UPLOADS_SUBDIR_REL_PATH;
    $breakdance_uploads_dir = path_join($wp_uploads_dir, wp_normalize_path($prefix));
...
function get_uploads_url(): string
{
    ...
    $prefix = BREAKDANCE_MODE === 'oxygen' ? OXYGEN_UPLOADS_SUBDIR_REL_PATH : BREAKDANCE_UPLOADS_SUBDIR_REL_PATH;
    $breakdance_uploads_url = path_join($wp_uploads_url, wp_normalize_path($prefix));
...
function show_unavailable_directories_admin_notice()
{
    ...
    $message = sprintf(
        'One or more directories required for <b>%s</b> to function' .
            ' properly are missing or aren\'t writable. <a href="%s" target="_blank">Click here</a> for more details.',
        __bdox('plugin_name'),
        admin_url('admin.php?page=' . __bdox('admin_page_settings_slug') . '&tab=tools#create_directories_row')
    );
