...
function check_all_required_directories(): array
{
    ...
    foreach (array_keys(get_buckets()) as $bucket_id) {
        ...
        // Try to auto create temp dir if it doesn't exist
        if (is_temp_dir($bucket_dir_abspath)) {
            create_directory($bucket_dir_abspath);
        }
 ...
 * @return bool
 */
function is_temp_dir(string $directory_path)
{
    return str_starts_with($directory_path, get_temp_dir());
}
 ...
 * @throws \Exception
 */
function get_file_contents_from_bucket($bucket, $filename)
{
    $file_path_with_bucket_path_prefix = path_join(get_bucket_abs_path($bucket), $filename);

    if (!file_exists($file_path_with_bucket_path_prefix)) {
        return make_fs_wp_error(
            \Breakdance\Filesystem\Consts::BREAKDANCE_FS_ERROR_FILE_DOES_NOT_EXIST,
            $file_path_with_bucket_path_prefix
        );
    }

    return file_get_contents($file_path_with_bucket_path_prefix);
}
